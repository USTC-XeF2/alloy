...
pub fn expand_api_payload(attr: TokenStream, item: TokenStream) -> Result<TokenStream> {
    ...
    let builder_tokens = if default_fields.is_empty() {
        ...
        quote! {
            ...
            impl<T> #builder_trait_ident for ::amira_core::bot::ApiRequest<'_, T, #struct_ident>
            where
                T: ::amira_core::ApiExecutor<Bot = #bot_ty>,
            {
    ...
    let ext_tokens = quote! {
        pub trait #ext_trait_ident {
            #(#struct_doc_attrs)*
            fn #action_ident(&self, #(#required_sig),*) -> ::amira_core::bot::ApiRequest<'_, Self, #struct_ident>
            where
                Self: ::amira_core::ApiExecutor<Bot = #bot_ty> + Sized;
        }

        impl<T> #ext_trait_ident for T
        where
            T: ::amira_core::ApiExecutor<Bot = #bot_ty> + Sized,
        {
            fn #action_ident(&self, #(#required_sig),*) -> ::amira_core::bot::ApiRequest<'_, Self, #struct_ident> {
                ::amira_core::bot::ApiRequest::new(
                    self,
                    #struct_ident {
                        #(#init_fields),*
                    },
    ...
    Ok(quote! {
        ...
        impl ::amira_core::bot::ApiPayload for #struct_ident {
