...
pub fn expand_event_root(attr: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    ...
    Ok(quote! {
        ...
        impl ::amira_core::EventRoot for #root_name {
            ...
            fn event_type(&self) -> ::amira_core::EventType {
                self.#data_field_ident.event_type()
            }

            fn scene(&self) -> Option<::amira_core::Scene> {
            ...
            fn rich_text(&self) -> ::amira_core::RichText {
                self.#data_field_ident.rich_text()
            }
        }

        impl ::amira_core::event::PlatformEvent for #root_name {
        ...
        return match scene.as_str() {
            "Private" => {
                ...
                Ok(arm_with_cfg(
                    spec,
                    quote! {
                        Self::#variant_ident { #user_ident, .. } => Some(::amira_core::Scene::Private {
            ...
            "Group" => {
                ...
                if let Some(user_id) = user_id_field(spec) {
                    ...
                    Ok(arm_with_cfg(
                        spec,
                        quote! {
                            Self::#variant_ident { #group_ident, #user_ident, .. } => Some(::amira_core::Scene::Group {
                    ...
                    Ok(arm_with_cfg(
                        spec,
                        quote! {
                            Self::#variant_ident { #group_ident, .. } => Some(::amira_core::Scene::Group {
                                group_id: #group_ident.to_string(),
                                user_id: None,
                            }),
                        },
            ...
            "Guild" => {
                ...
                Ok(arm_with_cfg(
                    spec,
                    quote! {
                        Self::#variant_ident { #guild_ident, .. } => Some(::amira_core::Scene::Guild {
    ...
    if spec.fields.is_empty() {
        if let Some(event_type) = &spec.view_attr.event_type {
            return arm_with_cfg(
                spec,
                quote! {
                    Self::#variant_ident => ::amira_core::EventType::#event_type,
                },
            );
        }
        return arm_with_cfg(
            spec,
            quote! {
                Self::#variant_ident => ::amira_core::EventType::Other,
            },
        );
    }
...
fn message_text_arms(spec: &VariantSpec) -> (TokenStream, TokenStream) {
    ...
    (
        ...
        arm_with_cfg(
            spec,
            quote! { Self::#variant_ident { .. } => ::amira_core::RichText::new(), },
        ),
    )
}
...
fn generate_view_tokens(enum_name: &Ident, parent_ty: &Type, spec: &VariantSpec) -> TokenStream {
    ...
    quote! {
        ...
        #(#variant_cfg_attrs)*
        impl ::amira_core::EventView for #view_name {
            type Root = <#parent_ty as ::amira_core::EventView>::Root;
