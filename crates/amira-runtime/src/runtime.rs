...
//! let runtime = AmiraRuntime::from_config(&config);
//! ```
...
/// use amira::prelude::*;
///
/// // Auto-loads config from amira.toml in current directory
/// let runtime = AmiraRuntime::new();
///
/// // Register an adapter (configured from amira.toml)
...
///
/// ```rust,ignore
/// let runtime = AmiraRuntime::builder()
...
impl AmiraRuntime {
    ...
    ///
    /// This will:
    /// 1. Search for `amira.toml` in the current directory
    ...
    ///
    /// ```rust,ignore
    /// use amira_runtime::{AmiraRuntime, config::ConfigLoader};
    ///
    /// let config = ConfigLoader::new()
    ///     .with_current_dir()
    ///     .load()?;
    ...
    ///
    /// Configuration is loaded from `amira.toml` under the adapter's name key,
