...
/// # Storage layout
///
/// The factory produces an `Arc<dyn Any + Send + Sync>` whose *inner value* is an
/// `Arc<dyn ServiceTrait>`.  The registry key is `TypeId::of::<dyn ServiceTrait>()`.
/// At retrieval time, the outer `Arc` is dereffed to `dyn Any` and
/// `downcast_ref::<Arc<dyn ServiceTrait>>()` recovers the inner trait-object Arc.
///
/// This double-Arc pattern lets the service map stay homogeneously typed while
/// still preserving the full trait-object interface for consumers.
///
...
pub struct Plugin {
    ...
    pub(crate) on_load_fn: Option<OnLoadFn>,
    pub(crate) on_unload_fn: Option<OnUnloadFn>,
...
impl Plugin {
    ...
    pub async fn on_load(&self, ctx: Arc<PluginLoadContext>) -> Result<(), BoxError> {
